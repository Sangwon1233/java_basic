...
//	private int cnt;
	...
	// 학생 등록
	public void add() {
		Integer no = next("학번", Integer.class);
		if(findBy(no) != null) {
			throw new RuntimeException("중복되지 않는 학번을 입력하세요");
		}
	
		String name = checkName(next("이름",String.class));
...
//		System.out.println("list()");
		int input =next("1.입력순 2. 학번순 3. 이름순 4. 석차순",Integer.class);
			//	(("1.입력순 2. 학번순 3. 이름순 4. 석차순), 1, 4);
		List<Student> tmp = null;
		tmp = new ArrayList<Student>();
...
//			System.out.println(students[i]);
	...
	public void modify() {
		...
		String name = next("이름", String.class, str->str.matches("^[가-힣]{2,4}"),"정확한 이름을 입력하시오" );
		//s.setName(checkName(nextInt("이름")));
//		int kor<=0&&kor>=100 
//		Integer kor = next("국어",Integer.class);
		Integer kor = next("국어",Integer.class, t-> t>=0 && t<=100,"정확한 점수를 입력하세요");
//		Integer eng = next("영어",Integer.class);
//		Integer mat= next("수학",Integer.class);
		Integer eng = next("영어",Integer.class, t-> t>=0 && t<=100,"정확한 점수를 입력하세요");
		Integer mat = next("수학",Integer.class, t-> t>=0 && t<=100,"정확한 점수를 입력하세요");
		 s.setName(name);
		 s.setKor(kor);
		 s.setEng(eng);
		 s.setMat(mat);
